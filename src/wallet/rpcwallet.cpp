...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    StartShutdown();
    return "Wallet encrypted; Polis Core server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
                            ...
                            "                              Those recipients will receive less polis than you enter in their corresponding amount field.\n"
    ...
    if (request.params.size() > 1) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid polis address");
...
extern UniValue dumpprivkey_polis(const JSONRPCRequest& request); // in rpcdump.cpp
extern UniValue importprivkey(const JSONRPCRequest& request);
extern UniValue importaddress(const JSONRPCRequest& request);
extern UniValue importpubkey(const JSONRPCRequest& request);
extern UniValue dumpwallet_polis(const JSONRPCRequest& request);
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "dumpprivkey",              &dumpprivkey_polis,        true,   {"address"}  },
    { "wallet",             "exportprivkey",            &dumpwallet_polis,         true,   {"filename"} },
