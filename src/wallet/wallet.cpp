    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fMasternodeMode && wtx.tx->vout[i].nValue == 1000*COIN && chainActive.Height()  < 57615) continue;
		if(fMasternodeMode && wtx.tx->vout[i].nValue == 2500*COIN && chainActive.Height() >= 57615) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    for (const auto& out : vCoins)
    {
        ...
        if(fMasternodeMode && out.tx->tx->vout[out.i].nValue == 1000*COIN && chainActive.Height() < 57615) continue; //masternode input
        if(fMasternodeMode && out.tx->tx->vout[out.i].nValue == 2500*COIN && chainActive.Height() >= 57615) continue; //masternode input
        if(nValueRet + out.tx->tx->vout[out.i].nValue <= nValueMax){
            CTxIn txin = CTxIn(out.tx->GetHash(),out.i);
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 aced."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-aced-address
                        CScript scriptChange;
